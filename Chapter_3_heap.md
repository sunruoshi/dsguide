...
$$A$$ 班同学们的初始信息被保存在一个数组中：

```cpp
string names[5] = {"Alice", "Bob", "Celine", "David", "Elizabeth"};
```

当然，我们可以对 $$A$$ 班的所有成员按年龄从小到大排序一次。但是一旦有新的成员加入，或者有老的成员离开，那么所有成员的年龄顺序就又乱了。那么在下次点名，就又需要重新排序一次。在 $$A$$ 班的成员数量很多的时候，这显得太低效了。
...
但是美中不足的是，二叉堆在某些场景下并不是太好用。例如，如果你想要知道所有学生中名字的字典序「第二小」的人是谁，你是没有办法直接从堆中得到的：你必须先取出堆顶的成员，然后等待堆把第二小的成员「送」到堆顶，但很多时候你只是想要查询一下，并不想要取出元素。如果你想要知道一个最小堆中的最大成员，你甚至得把堆清空，这并不是很方便。
