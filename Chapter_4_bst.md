...
除了指向父节点的指针域 $$F$$ 以外，根据不同的需求，为了方便使用，我们还可以给 $$BST$$ 添加一些额外的「域」，例如维护子树大小的 $$size$$ 域。关于这一部分的代码实现，请参考附录一中的内容。
...
那么又没有办法能让 $$BST$$ 能够自行调整自己的高度，使得内部节点的排列尽量的接近完全二叉树呢？

这种能够自行调整高度的二叉搜索树，就叫做「平衡树」。