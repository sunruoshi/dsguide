...
- 如果待插入的节点的 $$name$$ 值「小于等于」当前节点 $$cur$$ ，就递归的往「左子树」插入；
- 如果待插入的节点的 $$name$$ 值「大于」当前节点 $$cur$$ ，就递归的往「右子树」插入；
...
这里的形参 $$x$$ 是一个结构体指针，即节点本身，所以在此之前需要先使用 $$findNode()$$ 方法找到 $$x$$ 的位置。这就需要节点 $$x$$ 在 $$BST$$ 中必须「存在」。

但是很多时候，我们想要查找的「值」在树中不一定存在，但是我们仍然想要找到这个值在 $$BST$$ 中的前驱或者后继。这种时候，以上方法就不再适用了。

下面给出一种直接查找「值」的实现（该值不一定在树中存在）：
...
注意：以上代码返回的前驱是「严格小于」$$x$$ 的。如果想要包含值 $$x$$ ，就将以上代码第 $$3$$ 行中的大于改为大于等于即可。
...
注意：以上代码返回的后继是「严格大于」$$x$$ 的。如果想要包含值 $$x$$ ，就将以上代码第 $$3$$ 行中的小于改为小于等于即可。
