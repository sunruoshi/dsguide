...
其中除了 $$L$$ 和 $$R$$ 之外，我们添加了一个额外的指针域 $$F$$ ，指向节点的父节点，初始值为 $$NULL$$ 。

在整棵树中，只有根节点 $$root$$ 的父节点为 $$NULL$$ 。

接下来我们来实现伸展树的 $$rightRotate()$$ 和 $$leftRotate()$$ 操作。
...
在明确了旋转操作之后，我们接下来讨论伸展树的「伸展」。

## 5.2 伸展树的伸展操作

伸展树的伸展操作（$$splay$$）是这样一种操作：

> 伸展操作 $$splay(x)$$ 就是要将节点 $$x$$ 通过一系列的旋转移动到树根。
...
void splay(SplayNode* x, SplayNode* &root) {
    while (x->F) {
        ...
        SplayNode* z = y->F;
        if (!z) {
            if (x == y->L) rightRotate(x);       // Zig
            else leftRotate(x);                  // Zag
        } else {
            if (x == y->L && y == z->L) {        // Zig-Zig
                rightRotate(y);
                rightRotate(x);
            } else if (x == y->L && y == z->R) { // Zig-Zag
                rightRotate(x);
                leftRotate(x);
            } else if (x == y->R && y == z->R) { // Zag-Zag
                leftRotate(y);
                leftRotate(x);
            } else {                             // Zag-Zig
                leftRotate(x);
...
如果你理解了上面的内容，就会发现 $$splay()$$ 操作的代码实现是何其的简洁优雅，且非常符合直觉。需要注意的是，在将 $$x$$ 移动到根节点之后，要更新根节点 $$root$$ 为 $$x$$ 。
...
如果这样进行操作，自然而然的，节点被操作的次数越多，就会离根节点 $$root$$ 越近。而离根节点越近，下一次操作的时间开销就越低。同时，$$splay$$ 的次数越多，整棵树的「形态」就越平衡。
...
那么如何解决呢？对于伸展树来说，如果 $$splay()$$ 操作执行的次数越多，整棵树的形态就会越平衡。在以上代码中，只在插入节点时进行伸展操作是不够的。每次执行完 $$findPred()$$ 和 $$findSucc()$$ 操作之后，都要再执行一次 $$splay()$$ 操作：
...
从伸展树的设计上就可以知道，伸展树并非时刻保持平衡的，它并不是一棵平衡树。它的特点在于：
...
因为伸展树的特殊性质，在伸展树中可以很简单的实现一些特殊的方法：如将一棵伸展树在节点 $$x$$ 处分断为两棵伸展树的 $$split()$$ 方法，以及将两棵伸展树 $$x$$ 和 $$y$$ 合并为一棵伸展树的 $$join()$$ 方法。在此基础之上，伸展树可以非常简单的实现删除节点 的 $$deleteNode()$$ 方法。

关于这一部分的内容，请参考 [附录二](Extra\_2.md) 。